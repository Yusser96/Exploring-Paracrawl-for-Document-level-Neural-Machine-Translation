...
cp_path_sent2=$run_path/$data-sent2.checkpoints.$slang-$tlang
...
if [ $mode == "train" ]; then
  ...
  python train.py $bin_path_sent --save-dir $cp_path_sent --tensorboard-logdir $cp_path_sent --seed 444 --fp16 --num-workers 4 \
         ...
         --optimizer adam --adam-betas "(0.9, 0.98)" --lr 5e-04 --max-epoch 5 --lr-scheduler inverse_sqrt --warmup-updates 4000 \
         --criterion label_smoothed_cross_entropy --label-smoothing 0.1 --no-epoch-checkpoints \
         --max-tokens 4096 --update-freq 1 --validate-interval 1 --patience 10 --skip-invalid-size-inputs-valid-test \
  ...
  python train.py $bin_path_doc --save-dir $cp_path_doc --tensorboard-logdir $cp_path_doc --seed 444 --num-workers 4 \
         ...
         --lr 5e-04 --max-epoch 5 --lr-scheduler inverse_sqrt --warmup-updates 4000 \
         --criterion label_smoothed_cross_entropy --label-smoothing 0.1 --no-epoch-checkpoints \
         ...
         --doc-noise-mask 0.1 --doc-noise-epochs 40 --skip-invalid-size-inputs-valid-test > $run_path/train.$data-doc.$slang-$tlang.log 2>&1

  cp -r $cp_path_sent $cp_path_sent2
  echo `date`, Training sentence-level 2 model again...
  python train.py $bin_path_sent --save-dir $cp_path_sent2 --tensorboard-logdir $cp_path_sent2 --seed 444 --fp16 --num-workers 4 \
         --task translation_doc --source-lang $slang --target-lang $tlang --langs $doc_langs \
         --arch transformer_doc_base --doc-mode full --share-all-embeddings \
         --optimizer adam --adam-betas "(0.9, 0.98)" --lr 5e-04 --max-epoch 10 --lr-scheduler inverse_sqrt --warmup-updates 4000 \
          --restore-file $sent_model \
         --criterion label_smoothed_cross_entropy --label-smoothing 0.1 --no-epoch-checkpoints \
         --max-tokens 4096 --update-freq 1 --validate-interval 1 --patience 10 --skip-invalid-size-inputs-valid-test \
         > $run_path/train.$data-sent2.$slang-$tlang.log 2>&1

elif [ $mode == "test" ]; then
  mkdir -p $res_path
  echo `date`, Testing Doc-level model on documents test dataset...
  python -m fairseq_cli.generate $bin_path_doc --path $cp_path_doc/checkpoint_best.pt \
         ...
         --gen-output $res_path/doc_test.$data.$slang-$tlang --skip-invalid-size-inputs-valid-test > $run_path/doc_test.$data.$slang-$tlang.log 2>&1

  echo `date`, Testing Doc-level model on sentences test dataset...
  python -m fairseq_cli.generate $bin_path_sent --path $cp_path_doc/checkpoint_best.pt \
         --gen-subset test --batch-size 16 --beam 5 --max-len-a 1.2 --max-len-b 10 \
         --task translation_doc --source-lang $slang --target-lang $tlang --langs $doc_langs \
         --doc-mode partial --tokenizer moses --remove-bpe --sacrebleu \
         --gen-output $res_path/doc_sent_test.$data.$slang-$tlang --skip-invalid-size-inputs-valid-test > $run_path/doc_sent_test.$data.$slang-$tlang.log 2>&1

  echo `date`, Testing Sent-level model on sentences test dataset...
  python -m fairseq_cli.generate $bin_path_sent --path $cp_path_sent2/checkpoint_best.pt \
         --gen-subset test --batch-size 16 --beam 5 --max-len-a 1.2 --max-len-b 10 \
         --task translation_doc --source-lang $slang --target-lang $tlang --langs $doc_langs \
         --doc-mode partial --tokenizer moses --remove-bpe --sacrebleu \
         --gen-output $res_path/sent_test.$data.$slang-$tlang --skip-invalid-size-inputs-valid-test > $run_path/sent_test.$data.$slang-$tlang.log 2>&1
else
